...
struct AlertsListView: View {
    ...
    private var filtered: [SophosAlert] {
        ...
        if let cutoff = datePreset.startDate {
            list = list.filter { ($0.raisedDate ?? .distantPast) >= cutoff }
        }
        return list
    }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                // Row 2: Date preset pills (equal-width, full row)
                HStack(spacing: SophosTheme.Spacing.xs) {
                    ForEach(AlertsDatePreset.allCases, id: \.self) { preset in
                        Button {
                            ...
                            Text(preset.label)
                                .font(SophosTheme.Typography.footnote(datePreset == preset ? .semibold : .regular))
                                .foregroundColor(datePreset == preset ? .white : SophosTheme.Colors.textSecondary)
                                .frame(maxWidth: .infinity)
                                .padding(.vertical, SophosTheme.Spacing.xs)
                                .background(datePreset == preset ? SophosTheme.Colors.sophosBlue : SophosTheme.Colors.backgroundCard2)
...
enum AlertsDatePreset: String, CaseIterable {
    case today   = "Today"
